...
def weighted_avg_glove_vector(descr_list, emoji_2_embedding_lookup):
    '''
      Returns from a preprocessed list of the emoji description the weighted average embedding
    ...
    for token in nlp_object:
        if str(token) in emoji_2_embedding_lookup.keys():
            if token.pos_ == "NOUN":
                n_vectors += 4
                avg_vector += 4 * emoji_2_embedding_lookup[str(token)]
            elif token.pos == "PROPN":
                n_vectors += 2
                avg_vector += 2 * emoji_2_embedding_lookup[str(token)]
            else:
                n_vectors += 1
                avg_vector += 1 * emoji_2_embedding_lookup[str(token)]
        else:
            continue
...
def weighted_find_closest_emoji_emb(sentence,emoji_2_embedding_lookup, weighted_emoji_symb2emb_dic, \
    ...
    preprocessed_list = some_preprocessing(sentence)
    # Take the Avg of the GloVe Embedded Vectors
    embedded_sentence = weighted_avg_glove_vector(preprocessed_list, emoji_2_embedding_lookup)
    if embedded_sentence == "":
      return []

    if distance_type == "euclidean":
        closest_emojis = sorted(weighted_emoji_symb2emb_dic.keys(), key= lambda emoji_symbol:\
                            spatial.distance.euclidean(weighted_emoji_symb2emb_dic[emoji_symbol], embedded_sentence))
    elif distance_type == "cosine":
        closest_emojis = sorted(weighted_emoji_symb2emb_dic.keys(), key= lambda emoji_symbol:\
                            spatial.distance.cosine(weighted_emoji_symb2emb_dic[emoji_symbol], embedded_sentence))
    return closest_emojis
...
def weighted_translate_text(spacy_nlp_file, emoji_2_embedding_lookup, weighted_emoji_symb2emb_dic, distance_type="euclidean"):
    '''
      Returns the input spacy_file split in sentences with the closest emoji
    ...
    for num, sentence in enumerate(spacy_nlp_file.sents):
        print(f'{num}: {sentence}')
        closest_emojis = weighted_find_closest_emoji_emb(str(sentence), emoji_2_embedding_lookup, \
                                                         weighted_emoji_symb2emb_dic, distance_type)
        print(closest_emojis[:3])
        print("")
